...
class GetProductSearchListUIUseCaseTest {

    private lateinit var repository: ProductRepository
    private lateinit var mapper: ProductUIMapper
    private lateinit var useCase: GetProductSearchListUIUseCase

    @Before
    fun setUp() {
        repository = mockk()
        mapper = mockk()
        useCase = GetProductSearchListUIUseCase(repository, mapper)
    }
    ...
    fun `invoke emits Result_Success with mapped list when repository returns success`() = runBlocking {
        ...
        coEvery { repository.searchProducts("query") } returns flowOf(Result.Success(listOf(productResponse)))
    ...
    fun `invoke emits Result_Error when repository returns error`() = runBlocking {
        coEvery { repository.searchProducts("query") } returns flowOf(Result.Error(message = "Erro", code = 500))
